...
function endGame(message) {
  ...
  if (message.includes('venceu')) {
    ...
    chooseProductButton.onclick = () => {
      // Captura os parâmetros UTM da URL atual e os adiciona ao link
      const baseURL = 'http://cacaupascoa2025.site/pascoaparatodos2/verificacao/';
      const utmParameters = new URLSearchParams(window.location.search);
      const utmLink = `${baseURL}?${utmParameters.toString()}`; // Concatenando os parâmetros UTM com o link base
      window.location.href = utmLink; // Redireciona o usuário para a URL com os parâmetros
    };
  } else {
    loseSound.play();
    claimPrizeButton.style.display = 'none';
